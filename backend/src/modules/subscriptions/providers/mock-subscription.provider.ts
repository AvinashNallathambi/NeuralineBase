...
export class MockSubscriptionProvider implements SubscriptionProvider {
  ...
  parseWebhook(rawBody: string | Buffer, _signature: string): SubscriptionWebhookEvent {
    const rawString = typeof rawBody === 'string' ? rawBody : rawBody.toString('utf8');
    const event = JSON.parse(rawString) as Record<string, any>;
    ...
    const invoice = event.type?.startsWith('invoice.')
      ? {
          id: obj.id ?? `mock_inv_${Date.now()}`,
          amount: (obj.total ?? obj.amount ?? 0) / 100,
          currency: obj.currency ?? 'usd',
          status: (event.type === 'invoice.payment_failed' ? 'failed' : 'paid') as any,
          hostedInvoiceUrl: obj.hosted_invoice_url ?? null,
          paidAt: obj.status === 'paid' ? new Date() : null,
          failureReason: event.type === 'invoice.payment_failed' ? 'Mock payment failed' : null,
          paymentMethodId: obj.default_payment_method ?? null,
          periodStart: obj.period_start ? new Date(obj.period_start * 1000) : undefined,
          periodEnd: obj.period_end ? new Date(obj.period_end * 1000) : undefined,
        }
      : null;
  ...
  async attachPaymentMethod(
    ...
    // Support ACH bank account payment methods in mock mode (IDs starting with pm_mock_bank_)
    const isBank = pmId.includes('bank');
    const paymentMethod: PaymentMethodDetails = isBank
      ? {
