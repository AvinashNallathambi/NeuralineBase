...
const ClinicalPage: React.FC = () => {
  ...
  const [patientMap, setPatientMap] = useState<Record<string, { firstName: string; lastName: string; mrn?: string }>>({});
  ...
  const fetchPatients = async () => {
    try {
      ...
      const map: Record<string, { firstName: string; lastName: string; mrn?: string }> = {};
      result.data.forEach((p: any) => {
        map[p.id] = { firstName: p.firstName, lastName: p.lastName, mrn: p.mrn };
      });
      setPatientMap(map);
    } catch {
      // silent
  ...
  const columns: ColumnsType<Encounter> = [
    {
      ...
      render: (t) => (
        <Space direction="vertical" size={0}>
          <Text strong style={{ fontSize: 13 }}>{dayjs(t).format('MM/DD/YYYY')}</Text>
          <Text type="secondary" style={{ fontSize: 12 }}>{dayjs(t).format('h:mm A')}</Text>
          {dayjs(t).isToday() && <Tag color="green" style={{ fontSize: 10, lineHeight: '14px' }}>Today</Tag>}
        </Space>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (cc, record) => (
        <Space direction="vertical" size={0}>
          ...
          {!cc && record.visitReason && <Text type="secondary" style={{ fontSize: 12 }}>{record.visitReason}</Text>}
          {!cc && !record.visitReason && <Text type="secondary" style={{ fontSize: 12 }}>—</Text>}
        </Space>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (providerId) => {
        ...
        return provider ? (
          ...
          <Text type="secondary" style={{ fontSize: 12 }}>{providerId?.substring(0, 8)}</Text>
        );
      },
    },
    {
      ...
      render: (status, record) => (
        <Space direction="vertical" size={0}>
          <Tag color={statusColors[status]}>{status.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}</Tag>
          {record.isLocked && (
            <Tooltip title="Encounter locked">
              <Tag icon={<LockOutlined />} color="orange" style={{ fontSize: 11 }}>Locked</Tag>
            </Tooltip>
          )}
          {record.signedAt && !record.isLocked && (
            <Tag icon={<FileDoneOutlined />} color="green" style={{ fontSize: 11 }}>Signed</Tag>
          )}
        </Space>
      ),
    },
  ...
  const tabItems = [
    ...
    { key: 'today', label: <Space><CalendarOutlined />Today</Space> },
  ...
  const todayCount = data?.data.filter((e) => dayjs(e.startTime).isToday()).length || 0;
  const inProgressCount = data?.data.filter((e) => e.status === 'in_progress').length || 0;
  ...
  return (
    <div>
      ...
      <Row gutter={12} style={{ marginBottom: 16 }}>
        <Col xs={24} sm={6}>
          <Card size="small">
            <Statistic title="Today's Encounters" value={data?.data.filter((e) => dayjs(e.startTime).isToday()).length ?? 0} />
          </Card>
        </Col>
        <Col xs={24} sm={6}>
          <Card size="small">
            <Statistic title="In Progress" value={data?.data.filter((e) => e.status === 'in_progress').length ?? 0} valueStyle={{ color: '#fa8c16' }} />
          </Card>
        </Col>
        <Col xs={24} sm={6}>
          <Card size="small">
            <Statistic title="Completed" value={data?.data.filter((e) => e.status === 'completed').length ?? 0} valueStyle={{ color: '#52c41a' }} />
          </Card>
        </Col>
        <Col xs={24} sm={6}>
          <Card size="small">
      ...
      <Card>
        ...
        <Table
          ...
          rowClassName={(record) => {
            if (record.status === 'in_progress') return 'encounter-row-in-progress';
            return '';
          }}
          pagination={{
            current: page,
            ...
            showTotal: (total, range) => `${range[0]}-${range[1]} of ${total} encounters`,
            onChange: (p) => setPage(p),
          }}
          locale={{
            emptyText: (
              <div style={{ padding: 32, textAlign: 'center' }}>
                <CalendarOutlined style={{ fontSize: 40, color: '#d9d9d9', marginBottom: 8 }} />
